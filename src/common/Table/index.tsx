  ...
  render() {
    ...
    return (
      <div className={`${styles["table-container"]} ${className}`}>
        {/* thead */}
        <div className={styles["tb-th"]} onClick={this.clickRow.bind(this)}>
          {
            React.Children.map(children, (item: any, index) => {
              let { label, width } = item.props;
              return <div onClick={this.clickThCol.bind(this, index, label)} className={styles["col"]} style={{ width: width }} key={Math.random().toString(36).substr(2)}>
        ...
        {/* tbody */}
        <div className={styles["tb-tbody"]}>
          {
            data.map((item: any, index) => {
              return (
                <div onClick={this.clickRow.bind(this, item, index)} className={styles["tb-row"]} key={Math.random().toString(36).substr(2)} >
                  {
