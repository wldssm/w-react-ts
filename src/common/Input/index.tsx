...
class WInput extends Component<Props> {
  ...
  render() {
    let { value, type, showPwd, placeholder, maxLength, className, autoComplete, leftNode, rightNode, width } = this.props;
    return (
      <div className={`${styles["i-txt-box"]} ${className}`} style={{ width: width }} >
        {leftNode}
        <input type={(type && !showPwd) ? type : 'text'} value={value} autoComplete={autoComplete}
